...
async function loadTailwind() {
  const cachedVersion = await fs.readFile(tailwindCachePath, 'utf8').catch(() => {})
  if (cachedVersion) {
    return cachedVersion
  }
  const css = await fetch('https://cdn.jsdelivr.net/npm/tailwindcss@1.x.x/dist/tailwind.min.css').then(res => res.text());
 ...
 */
async function main() {
  const files = (await fs.readdir(distDir)).filter(
    f => f.endsWith(".html")
  );
